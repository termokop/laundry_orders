  ...
  hotelData.value.rooms.forEach(currentRoom => {
    if(currentRoom.status === "Stay Over") {
      Object.keys(linens_for_stayOver[currentRoom.room_type]).forEach(key => {
        linensNeeded.value[parseInt(currentRoom.room_number.substring(0,2)).toString()][key] = (linensNeeded.value[parseInt(currentRoom.room_number.substring(0,2)).toString()][key] || 0) + linens_for_stayOver[currentRoom.room_type][key]
      })
    } else if(currentRoom.status === "Check Out") {
      Object.keys(linens_for_checkOut[currentRoom.room_type]).forEach(key => {
        linensNeeded.value[parseInt(currentRoom.room_number.substring(0,2)).toString()][key] = (linensNeeded.value[parseInt(currentRoom.room_number.substring(0,2)).toString()][key] || 0) + linens_for_checkOut[currentRoom.room_type][key]
      })
    }
  })
